...
def generate_processes(num_processes: int, display=False) -> (List[EchoElectionProcess], MessageManager):
    ...
    # Create a tree
    processes = [EchoElectionProcess(1, msg_manager)]
    nodes = [EchoElectionProcess(x, msg_manager) for x in range(2, num_processes + 1)]
...
def find_tree(processes: List[EchoElectionProcess], display=False, execution_num=None) -> Tree:
    """Finds the tree from processes after the echo election algorithm has been executed"""
    ...
    if display:
        # Display the tree
        title = "Tree generated by echo election algorithm"
        if execution_num:
...
def execute_echo_election_algorithm(processes: List[EchoElectionProcess], msg_manager: MessageManager,
                                    num_initiator: int, display_logs: bool = True, execution_num: int = None):
    # Initialise message manager and processes
    msg_manager.reset()
    msg_manager.initialise(processes)
    initiator = 0
    for process in processes:
        process.reset()  # If process has already been used
        if initiator < num_initiator:
            # Create initiator
    ...
    while len([process for process in processes if not process.decide]) > 0:
        # Wait until processes have decided
        pass
    ...
    tree = find_tree(processes, display=True, execution_num=execution_num)
    tree.show()
...
def run_echo_election_algorithm(num_processes: int, num_initiator: int, display_logs: bool = True,
    ...
    for i in range(num_executions):
        print(f"Running echo election algorithm ({i + 1} / {num_executions}) ...\n")
        execute_echo_election_algorithm(processes=processes,
                                        msg_manager=msg_manager,
                                        num_initiator=num_initiator,
